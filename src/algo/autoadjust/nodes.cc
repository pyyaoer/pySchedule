...
void Gate::HandleMessage_Request(std::shared_ptr<RequestMessage> msg) {
  ...
  // use parameters automatically
  {
    ...
    rtag_[r.tenant] = std::max(rtag_[r.tenant] + rho_[r.tenant]*dt / TENANT_RESERVATION, dnow);
    ltag_[r.tenant] = std::max(ltag_[r.tenant] + delta_[r.tenant]*dt / TENANT_LIMIT, dnow);
